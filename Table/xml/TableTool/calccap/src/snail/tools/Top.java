...
public class Top extends Tool {

	@Override
	public void build(Options options) {
		options.add(Jmxc.options());
		final String [] [] _options_ = {
				{ "-sc", "nsClass", "namespace for class. order is meaning. e.g. \"mkio.;mkdb.\"", "" },
				{ "-sl", "nsLock", "namespace for lock. order is meaning. e.g. \"mkio.;mkdb.\"", "" },
				{ "-r", "period", "period(millisecond) of top.", "1500"},
				{ "-n", "limit", "print lines number.", "10"},
	...
	private static class OrderBy implements java.util.Comparator<Entry<String, AtomicInteger>> {
		OrderBy() {
		}
		...
		public int compare(Entry<String, AtomicInteger> o1, Entry<String, AtomicInteger> o2) {
			...
			switch (magic) {
			case 1: return -1; // o1 is lock, o2 is not.
			case 2: return +1; // o2 is lock, o1 is not.
			case 3: case 0: break; // both are lock or not.
			}
					...
					if (isLockEntry(entry)) {
						if (limitLock < limit) {
							if (limitLock == 0)
								System.out.println();
							++ limitLock;
							System.out.println(Tool.format(4, entry.getValue().get()) + entry.getKey());
						} if (limitLock == limit) {
							++ limitLock;
							System.out.println(" - ...");
						}
						...
						if (limitClass < limit) {
							if (limitClass == 0)
								System.out.println();
							++ limitClass;
							System.out.println(Tool.format(5, entry.getValue().get()) + entry.getKey());
						} if (limitClass == limit) {
							++ limitClass;
							System.out.println("...");
						}
	...
	public static Map<String, AtomicInteger> top(Jmxc jmxc, String nsClass, String nsLock) throws Exception {
		final Object [] params = new Object[]{ nsClass, nsLock };
		return (Map<String, AtomicInteger>)jmxc.invoke("mkdb:type=Mkdb", "top", params, signature);
	}
}
