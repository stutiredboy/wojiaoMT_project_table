...
input_format *open_audio_file(FILE *in, oe_enc_opt *opt)
{
    ...
    while(formats[j].id_func)
    {
        ...
        if(size > buf_filled)
        {
			...
			ret = ReadFromWav(buf + buf_filled, buf_size - buf_filled);
            buf_filled += ret;
...
int aiff_open(FILE *in, oe_enc_opt *opt, unsigned char *buf, int buflen)
{
        ...
        (format.samplesize == 16 || format.samplesize == 8))
    {
        ...
        aiff->channel_permute = (int*)malloc(aiff->channels * sizeof(int));
        if (aiff->channels <= 6)
...
long wav_read(void *in, float **buffer, int samples)
{
	...
	unsigned char *buf = (unsigned char*)alloca(samples*sampbyte*f->channels);
    //long bytes_read = fread(buf, 1, samples*sampbyte*f->channels, f->f);
	long bytes_read = ReadFromWav(buf, samples*sampbyte*f->channels);
    int i,j;
    long realsamples;
