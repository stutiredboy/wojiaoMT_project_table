...
public class ${classname} <#if baseclass! !="" > extends ${baseclass}<#else>implements mytools.ConvMain.Checkable</#if> <#if xlsfiles[0] != "" && baseclass! == "" >,Comparable<${classname}></#if>{
	...
	private static java.util.Map<Integer,${classname}> doconv(String confDir) {		
		...
		<#list xlsfiles as file>
			try {
			String filename=confDir+"/${file}"+".xlsx";
			java.io.File xfile=new java.io.File(filename);
			if (!xfile.exists()) {
				filename=confDir+"/${file}"+".xlsm";
				xfile=new java.io.File(filename);
			}
			
			if(!xfile.canRead()) {
				logger.error("无法读取"+xfile.getAbsolutePath());
				logger.error("文件名是:"+filename);
				return result;
			}
	...
	<#else>
		...
		<#if myvar.prefixMapping?size == 0>
			...
			for(int i=0;;i++){
				try{
					if (this.${myvar.name} == null)
						this.${myvar.name}=${myvar.initValue};
					this.${myvar.name}.add(new ${myvar.valueType}(row,collnames,"",i));
				}catch(IndexOutOfBoundsException ex){
					break;
				}
			}			
